...
class Graph {
  ...
  addEdge(obj, obj2, data) {
    ...
    if (!obj.adjacentTo(obj2)) {
      ...
      adjsObj[obj2.id] = adjsObj2[obj.id] = new Graph.Edge(
        obj,
        obj2,
        data,
        this.Edge,
...
Graph.Node = class {
  ...
  toString() {
    return (
      "Node(" +
      JSON.stringify([this.id, this.name, this.data, this.adjacencies]) +
      ")"
    );
  }
};
...
Graph.Edge = class {
  ...
  static fromJSON(graph, edgeJSON) {
    return new Graph.Edge(
      graph.get(edgeJSON.nodeFrom),
