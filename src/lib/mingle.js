// Credit: https://github.com/philogb/mingle
...
Graph.Render = {
  ...
  renderQuadratic: function (ctx, edges, options) {
    ...
    for (i = 0, l = edges.length; i < l; ++i) {
      ...
      nodeStart = e[0].node;
      ctx.lineWidth =
        (Math.max(1, nodeStart.data.weight) || 1) * (options.scale || 1);
      ...
      ctx.globalAlpha =
        nodeStart.data.alpha == undefined ? 1 : nodeStart.data.alpha;
      ctx.beginPath();
      for (j = 0, n = e.length; j < n; ++j) {
        posItem = e[j];
  ...
  adjustPosition: function (id, posItem, pos, margin, delta) {
    ...
    if (nodeArray) {
      ...
      pos = $add(
        pos,
        $mult(
          (lengthBefore - (lengthBefore + lengthAfter) / 2) *
            Math.min(epsilon, delta),
          posItem.normal
        )
      );
    }
...
class Bundler {
  ...
  getCombinedNode(node1, node2, data) {
    ...
    data = data || {};
    data.nodes = nodes;
    data.nodeArray = (node1.data.nodeArray || []).concat(
      node2.data.nodeArray || []
    );
